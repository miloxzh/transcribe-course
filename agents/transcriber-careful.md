---
name: transcriber-careful
description: Fallback writer for the /transcribe-course skill (Opus, medium effort) — same job as `transcriber`, used when a draft's coverage stays below the threshold or the gate fails twice, or when the user asks for the careful version. Invoke only with the brief from the skill's references/agent-brief.md.
model: opus
effort: medium
