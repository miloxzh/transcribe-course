# Writer prompt (fallback when the `transcriber` agent is not installed)

Use this as the system-level instruction when delegating to a `general-purpose` subagent with
`model: sonnet` (or `opus` for the careful version). It is the same text as `agents/transcriber.md`.
Put it at the top of the Agent prompt, then the filled brief from `agent-brief.md` below it.
